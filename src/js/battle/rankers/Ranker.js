    ...
    function rankerObject() {
      ...
      this.initPokemonList = function (cp) {
        ...
        if (cup.excludeLowPokemon && gm.rankings["alloverall" + cp]) {
          ...
          var lowPokemon = gm.rankings["alloverall" + cp].filter(
            (ranking) => ranking.score < 70
          );

          for (var i = 0; i < lowPokemon.length; i++) {
            if (
              pokemonList.findIndex(
                (r) => r.speciesId == lowPokemon[i].speciesId
              ) > -1
            ) {
              pokemonList.splice(
                pokemonList.findIndex(
            ...
            if (
              targets.findIndex((r) => r.speciesId == lowPokemon[i].speciesId) >
              -1
            ) {
              targets.splice(
                targets.findIndex(
      ...
      this.rankLoop = function (cp, cup, callback, data) {
        ...
        var rankingInterval = setInterval(function () {
          if (
            rankingCombinations.length == currentRankings &&
            rankingCombinations.length > 0
          ) {
            currentRankings--;
            ...
            var r = self.rank(
              rankingCombinations[0].league,
              rankingCombinations[0].scenario
            );
            allResults.push(r);
      ...
      this.rank = function (league, scenario) {
        ...
        for (var i = 0; i < rankCount; i++) {
          ...
          for (var n = 0; n < targets.length; n++) {
            ...
            if (moveSelectMode == "auto") {
              ...
              if (scenario.energy[0] == 0) {
                ...
                var fastMoveCount = Math.floor(
                  (scenario.energy[0] * 500) / pokemon.fastMove.cooldown
                );
                if (fastMoveCount == 0) {
                  fastMoveCount = 1;
                }
                ...
                pokemon.startEnergy = Math.min(
                  pokemon.fastMove.energyGain * fastMoveCount,
                  100
                );
              }
              ...
              if (scenario.energy[1] == 0) {
                ...
                opponent.startEnergy = Math.min(
                  opponent.fastMove.energyGain * fastMoveCount,
                  100
                );
              }
              ...
              damageRating =
                (opponent.stats.hp - opponent.hp) / opponent.stats.hp;

              opHealthRating = opponent.hp / opponent.stats.hp;
              opDamageRating =
                (pokemon.stats.hp - pokemon.hp) / pokemon.stats.hp;
            ...
            rankObj.matches.push({
              ...
              moveUsage: pokemon.generateMoveUsage(
                opponent,
                opponent.weightModifier
              ),
              oppMoveUsage: opponent.generateMoveUsage(
                pokemon,
                pokemon.weightModifier
              ),
            });
          ...
          for (var j = 0; j < rankObj.matches.length; j++) {
            ...
            for (var k = 0; k < chargedMoves.length; k++) {
              for (var l = 0; l < moveUsage.chargedMoves.length; l++) {
                if (
                  chargedMoves[k].moveId == moveUsage.chargedMoves[l].moveId
                ) {
          ...
          fastMoves.sort((a, b) =>
            a.uses > b.uses ? -1 : b.uses > a.uses ? 1 : 0
          );
          ...
          for (var i = 0; i < rankCount; i++) {
            ...
            for (var j = 0; j < matches.length; j++) {
              ...
              if (pokemonList.length == targets.length) {
                weight = Math.pow(
                  Math.max(
                    rankings[j].scores[n] / bestScore -
                      (0.1 + rankCutoffIncrease * n),
                    0
                  ),
                  rankWeightExponent
                );
              ...
              if (matches[j].adjRating > 700) {
                matches[j].adjRating =
                  700 + Math.pow(matches[j].adjRating - 700, 0.5);
              }
              ...
              if (matches[j].adjRating < 300) {
                ...
                matches[j].adjRating = Math.pow(
                  300,
                  (curveAdjustment + matches[j].adjRating) /
                    (300 + curveAdjustment)
                );
              }
        ...
        for (var i = 0; i < rankCount; i++) {
          ...
          if (scenario.slug == "chargers") {
            var fastMoveDPT =
              (pokemon.fastMove.power *
                pokemon.fastMove.stab *
                pokemon.shadowAtkMult *
                (pokemon.stats.atk / 100)) /
              (pokemon.fastMove.cooldown / 500);
            var maximumEnergyRemaining =
              100 -
              Math.min.apply(
                Math,
                pokemon.activeChargedMoves.map(function (m) {
                  return m.energy;
                })
              );

            rankings[i].score *= Math.pow(
              Math.pow(maximumEnergyRemaining / 100, 1 / 2) *
                Math.pow(fastMoveDPT / 5, 1 / 6),
              1 / 6
            );
          }
        ...
        if (cup.name != "custom") {
          ...
          // For custom rankings, JSON will be logged by CustomRankingInterface
          // which handles all scenarios together
        }
      ...
      function generateURLMoveStr(pokemon) {
        ...
        var chargedMove1Index =
          pokemon.chargedMovePool.indexOf(pokemon.chargedMoves[0]) + 1;
        var chargedMove2Index =
          pokemon.chargedMovePool.indexOf(pokemon.chargedMoves[1]) + 1;

        moveStr =
          fastMoveIndex + "-" + chargedMove1Index + "-" + chargedMove2Index;
