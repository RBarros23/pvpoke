...
function interfaceObject() {
  ...
  // Default scenario for display (will be set to first scenario from gamemaster)
  var displayScenario = {
    slug: "leads",
    shields: [1, 1],
    energy: [0, 0],
  ...
  this.init = function () {
    ...
    // Initialize scenarios from gamemaster
    scenarios = data.rankingScenarios;
    if (scenarios && scenarios.length > 0) {
      displayScenario = scenarios[0]; // Default to first scenario (leads)
    }

    // Also include overall and consistency as derived categories
    var derivedCategories = [
      { slug: "overall", derived: true },
      { slug: "consistency", derived: true },
    ];
    ...
    $(".subject-shield-select, .target-shield-select").on(
      "change",
      changeScenarioShields
    );
    $(".subject-turns, .target-turns").on(
      "keyup, change",
      changeScenarioEnergy
    );
    ...
    $("body").on(
      "click",
      ".field-section.type .select-all, .field-section.type .deselect-all",
      typeSelectAll
    );
    ...
    $.getJSON(
      webRoot + "data/gamemaster/cups/archive/cups.json?v=" + siteVersion,
      function (archive) {
  ...
  this.updateFilterValues = function ($el) {
    ...
    switch (filter.filterType) {
      ...
      case "dex":
        filter.values = [
          parseInt($el.find("input.start-range").val()),
          parseInt($el.find("input.end-range").val()),
        ];
        break;
  ...
  this.addFilterFromData = function (category, data, index) {
    ...
    $filter
      .find('.filter-type option[value="' + data.filterType + '"]')
      .prop("selected", "selected");
  ...
  function selectLeague(e) {
    ...
    var levelCap = parseInt(
      $(".league-select option:selected").attr("level-cap")
    );
  ...
  // NOTE: These settings are now ignored - we always run all standard scenarios
  ...
  function changeScenarioShields(e) {
    ...
    if (allowed.indexOf(val) > -1) {
      displayScenario.shields[index] = val;
    }
  }

  // Event handler for changing the amount of energy in the ranking scenario
  // NOTE: These settings are now ignored - we always run all standard scenarios
  ...
  function changeScenarioEnergy(e) {
    ...
    displayScenario.energy[index] = val;
  }
  ...
  function generateRankings(e, data) {
    ...
    setTimeout(function () {
      ...
      if (!data) {
        // Generate movesets (only run first scenario for this)
        ranker.setMoveSelectMode("auto");
        ranker.setScenarioOverrides([scenarios[0]]);
        ranker.rankLoop(battle.getCP(true), cup, self.receiveRankingData);
      } else {
        // Generate rankings with movesets established - run ALL scenarios
        ranker.setMoveSelectMode("force");
        ranker.setScenarioOverrides(scenarios); // Pass all scenarios
        rankingInterface.setScenario(scenarios[0]); // Display first scenario (leads)
        ranker.rankLoop(
          battle.getCP(true),
          cup,
          self.receiveRankingData,
          data[0]
        );
      }
    }, 250);
  ...
  function importIdsFromCustomGroup(e) {
    ...
    var $groupOptions = $(".poke.multi")
      .first()
      .find(".quick-fill-select option[type='custom']");
  ...
  // Calculate overall rankings from battle scenarios
  this.calculateOverallRankings = function (scenarioData) {
    var categories = ["leads", "closers", "switches", "chargers", "attackers"];
    var rankings = [];

    // Get all Pokémon from first scenario
    var baseData = scenarioData[0];

    for (var i = 0; i < baseData.length; i++) {
      var rankObj = {
        speciesId: baseData[i].speciesId,
        speciesName: baseData[i].speciesName,
        rating: 0,
        score: 0,
        moveset: baseData[i].moveset,
        matchups: baseData[i].matchups,
        counters: baseData[i].counters,
        moves: baseData[i].moves,
        scores: [],
      };

      // Collect scores from all 5 scenarios
      for (var j = 0; j < Math.min(scenarioData.length, 5); j++) {
        var scenarioRanking = scenarioData[j];
        var pokemonRanking = scenarioRanking.find(
          (p) => p.speciesId === baseData[i].speciesId
        );
        if (pokemonRanking) {
          rankObj.scores.push(pokemonRanking.score);
        }
      }

      // Calculate overall score (weighted average)
      if (rankObj.scores.length === 5) {
        var sortedScores = [...rankObj.scores].sort((a, b) => b - a);

        // Weight: top 2 scores get 9/20, bottom 2 get 1/20, middle gets 8/20
        rankObj.score = Math.floor(
          sortedScores[0] * 0.45 + // Best score
            sortedScores[1] * 0.45 + // Second best
            sortedScores[2] * 0.08 + // Middle
            sortedScores[3] * 0.01 + // Fourth
            sortedScores[4] * 0.01 // Worst
        );
      }
  ...
  // Calculate consistency rankings from battle scenarios
  this.calculateConsistencyRankings = function (scenarioData) {
    var rankings = [];
    var baseData = scenarioData[0];
    ...
    for (var i = 0; i < baseData.length; i++) {
      ...
      // Create Pokémon object to calculate consistency
      var pokemon = new Pokemon(baseData[i].speciesId, 0, battle);
      if (pokemon.initialize) {
        pokemon.initialize(true);
        pokemon.selectMove("fast", baseData[i].moveset[0]);
        pokemon.selectMove("charged", baseData[i].moveset[1], 0);
        if (baseData[i].moveset.length > 2) {
          pokemon.selectMove("charged", baseData[i].moveset[2], 1);
        }

        // Calculate consistency score
        rankObj.score = pokemon.calculateConsistency();
      }
